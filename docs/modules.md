...
entryは特定のデータモデルの操作が二つ以上あったり、操作時にデフォルト値を適用したりと、ロジックが複雑になる場合に用意する。  
CRUDというと、createとupdateで二つできるので、CRUDな場合は用意するが、eventモデルなど、登録のみの場合はcoreをそのまま利用する。  

### exit
entryと対になる存在で、出力値を表す。  
coreの値以外にも、項目を持っている場合には必ず用意する。  
特に依存している集約モデルを持たなければならない場合、coreは対象の集約のキーのみ保持しているので、参照を持つためにexitを用意する。  
逆にidなどの出力に含めたくない情報は、構造体のtagで`json:"-"`とomitできるので、それはcoreでも表現可能。  

### core  
ビジネスロジックで扱う純粋なデータモデル。何にも依存しない  
常に用意する。recordがない場合はDB定義、entryがない場合は入力定義、exitがない場合は出力定義も含める。  
entryもexitもない場合は、入力と出力の定義は同じにならなければならず、その条件から外れる場合はentryかexitを用意する。  
参照等価な関数はこのモジュールに集める。  
